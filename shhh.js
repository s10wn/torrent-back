...
async function setupServer() {
    ...
    app.post('/add-torrent', async (req, res) => {
        const { torrentId, season, episode, isFilm } = req.body;
        ...
        if (existingTorrent) {
            ...
            if (file) {
                ...
                res.json({ url: `http://localhost:3001/search/${encodeURIComponent(torrentId)}/${season}/${episode}` });
            ...
            client.add(torrentId, torrent => {
                ...
                if (file) {
                    ...
                    torrent.files.map((file) => episodeNames.push(file.name))

                    console.log(episodeNames)
                    res.json({ url: `http://localhost:3001/search/${encodeURIComponent(torrentId)}/${season}/${episode}`, episodeNames });
    ...
    app.get('/search/:torrentId/:season/:episode', async (req, res) => {

        const { torrentId, season, episode } = req.params;
        const torrent = await client.get(decodeURIComponent(torrentId));
        if (torrent && torrent.files.length) {
            const seasonEpisodeRegex = new RegExp(`[sS]${season.padStart(2, '0')}[eE]${episode.padStart(2, '0')}`);

            const file = torrent.files.find(file =>
                seasonEpisodeRegex.test(file.name) &&
